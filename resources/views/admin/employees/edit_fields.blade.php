...
<div class="form-group col-sm-6">
    ...
    {!! Form::select('department_id', $departments, '', ['class' => 'form-control','placeholder'=>'Please Select ...' ,'disabled' ]) !!}
    @push('scripts')
        <script>
            let the_location;
                ...
                $.ajax({
                    ...
                    success: (response)=>{
                        ...
                        response.message[0].employees.forEach(employee=>{
                            $('#supervisor_id').append('<option ></option>')
                            $('#supervisor_id').append('<option value='+employee.employee_id+'>'+employee.first_name+' '+employee.last_name+'</option>')
                        })
                    } ,
                });
            })
